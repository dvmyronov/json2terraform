...
echo "$json" | jq . | sed "1s/{/resource \"${name}\" \"${id}\" {/" | sed -n '
    ...
    /{/ {x;s/$/{/;x};                           #If { is found in pattern space, swap hold space with pattern space and append {, then swap back
    /\[/ {x;s/$/\[/;x};                         #If [ is found in pattern space, swap hold space with pattern space and append [, then swap back             
    /}/ {x;s/.$//;x};                           #If } is found in pattern space, swap hold space with pattern space, remove the last character, and swap back.
    /\]/ {x;s/.$//;x};                          #If ] is found in pattern space, swap hold space with pattern space, remove the last character, and swap back.
